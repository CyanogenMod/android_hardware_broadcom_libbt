...
//on manta this causes a crash due to an overrun elsewhere. Sad as it is, if we just do not do the free here we'll just leak less than 4K each time BT is turned on. And since when it is turned off, the process dies, this realy only costs us 4K total. I'm willing to part with 4K to avoid debugging bluedroid
//    if (bt_vendor_cbacks)
