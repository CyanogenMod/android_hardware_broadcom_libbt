...
**
*******************************************************************************/
char* hw_samsung_bluetooth_type()
{
    char buf[10];
...
static uint8_t hw_config_findpatch(char *p_chip_id_str)
{
    ...
    if ((dirp = opendir(fw_patchfile_path)) != NULL)
    {
        ...
        while ((dp = readdir(dirp)) != NULL)
        {
            ...
            static char samsung_patchfile_name[FW_PATCHFILE_PATH_MAXLEN] = { 0 };
            ...
            if ((hw_strncmp(dp->d_name, samsung_patchfile_name, strlen(samsung_patchfile_name)) \
                ) == 0)
